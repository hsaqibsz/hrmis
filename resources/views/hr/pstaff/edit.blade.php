...
@section('page-content')
    ...
    <div class="block" >
        ...
        <div class="block-title">
            <h2><strong>Edit</strong> {{$pstaff->user->name}} &nbsp; {{$pstaff->user->lastname}}'s contract information </h2>
        </div>
           <form id="basic-wizard" action="{{route('pstaff.update', $pstaff->user_id)}}" method="post"   class="form-horizontal form-bordered ui-formwizard" enctype="multipart/form-data">
               @csrf
            {{ method_field('patch') }}
           ...
           <input type="hidden" name="project_id" value="{{$pstaff->project_id}}">
           <input type="hidden" name="donor_id" value="{{$pstaff->donor_id}}">
            ...
            <div class="col-md-6">
                <select id="user_id" name="user_id" class="select-select2 select2-hidden-accessible" style="width: 100%;" data-placeholder="Choose one.." tabindex="-1" aria-hidden="true">
                    ...
                    <option value="{{$user->id}}" @if($pstaff->user_id == $user->id) selected @endif> {{$user->name}} &nbsp; {{$user->lastname}} </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6">
                <select id="region_id" name="region_id" class="select-select2 select2-hidden-accessible" style="width: 100%;" data-placeholder="Choose one.." tabindex="-1" aria-hidden="true">
                    ...
                    <option value="{{$region->id}}" @if($pstaff->region_id == $region->id) selected @endif> {{$region->name}}   </option>
                    @endforeach
                </select>
        ...
        <div class="col-md-6">
            <select id="province_id" name="province_id" class="select-select2 select2-hidden-accessible" style="width: 100%;" data-placeholder="Choose one.." tabindex="-1" aria-hidden="true">
                ...
                <option value="{{$province->id}}" @if($pstaff->province_id == $province->id) selected @endif> {{$province->name}}   </option>
                @endforeach
            </select>
    ...
    <div class="form-group">
        ...
        <div class="col-md-6">
            <select id="position_id" name="position_id" class="select-select2 select2-hidden-accessible" style="width: 100%;" data-placeholder="Choose one.." tabindex="-1" aria-hidden="true">
                ...
                <option value="{{$position->id}}" @if($pstaff->position_id == $position->id) selected @endif> {{$position->name}}   </option>
                @endforeach
            </select>
...
<div class="form-group">
    ...
    <div class="col-md-6">
  <label>Direct  <input type="radio" name="DorS" id="DorS" value="1" @if($pstaff->DorS == 1) checked @endif onchange="percentage();"> </label> &nbsp; &nbsp; <label>Support  <input type="radio" name="DorS" id="DorS" @if($pstaff->DorS == 0) checked @endif  onchange="percentage();" value="0"> </label>
    </div> 
</div>  
...
<div class="col-md-6">
    <select   name="percentage" class="select-select2 select2-hidden-accessible" style="width: 100%; " data-placeholder="Choose one.." tabindex="-1" aria-hidden="true">
        ...
        <?php for ($i=1; $i <= 100; $i++) { ?>
<option value="<?PHP echo $i; ?>" @if($pstaff->percentage == $i) selected @endif> <?PHP echo $i; ?> &nbsp; % </option>
<?php   }  ?>
...
<div class="form-group">
    ...
    <div class="col-md-6">
    <input type="text" name="title_this_project" class="form-control" placeholder="Title in this project" value="{{ $pstaff->title_this_project or old('title_this_project') }}">
</div> 
</div>            
...
<div class="form-group">
    ...
    <div class="col-md-6">
    <input type="number" name="salary" class="form-control" placeholder="salary of the project" value="{{ $pstaff->salary or old('salary') }}">
</div> 
</div>            
    ...
    <div class="form-group">
        ...
        <div class="col-md-6">
            <select id="currency_id" name="currency_id" class="select-select2 select2-hidden-accessible" style="width: 100%;" data-placeholder="Choose one.." tabindex="-1" aria-hidden="true">
                ...
                <option value="{{$currency->id}}" @if($pstaff->currency_id == $currency->id) selected @endif> {{$currency->name}}   </option>
                @endforeach
            </select>
    ...
    <div class="form-group">
        ...
        <div class="col-md-6">
        <input type="text" id="start_date" name="start_date" class="form-control input-datepicker" data-date-format="yyyy-mm-dd" placeholder="yyyy-mm-dd" value="{{ $pstaff->start_date or old('start_date')}}"> 
        </div>
    </div> 
...
<div class="form-group">
        ...
        <div class="col-md-6">
        <input type="text" id="completion_date" name="completion_date" class="form-control input-datepicker" data-date-format="yyyy-mm-dd" placeholder="yyyy-mm-dd" value="{{ $pstaff->start_date or old('completion_date')}}"> 
        </div>
    </div> 
